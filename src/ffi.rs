...
mod ffi_bridge {
	...
	/// Planar section polylines in the section plane's own 2D frame.
	struct PlaneSectionData {
		/// Flat XY coordinates, wire after wire.
		points: Vec<f64>,
		/// Point count of each wire, indexing into `points` pairs.
		wire_sizes: Vec<u32>,
		/// 1 for a closed wire, 0 for an open one; parallel to `wire_sizes`.
		wire_closed: Vec<u8>,
		success: bool,
	}
	...
	unsafe extern "C++" {
		...
		fn transform_scale(shape: &TopoDS_Shape, cx: f64, cy: f64, cz: f64, factor: f64, out_topology_history: &mut HistoryData) -> UniquePtr<TopoDS_Shape>;

		fn transform_mirror(shape: &TopoDS_Shape, ox: f64, oy: f64, oz: f64, nx: f64, ny: f64, nz: f64, out_topology_history: &mut HistoryData) -> UniquePtr<TopoDS_Shape>;
